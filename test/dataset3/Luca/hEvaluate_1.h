// Author: Luca Carlon
// Info: To havo informations go to www.planet-source-code.com
// Finished: 31.07.2003
...
// This function fill the structure structure_parenthesis with informations about the levels and positions of
// open and close parenthesis in the string passed as parameters.
...
void Solve(char sExpression[])
{
	...
	for (int iCounter = 0; parenthesis[iCounter].iClose != 0; iCounter++)
	{
		...
		if (IsLetter(sExpression, iCounter, true) == true)
		{
			...
			switch (iFunction)
			{
				...
				case 28:
				{
					...
					double dSecondArgument = FindOperator(sExpression, (iEndSecondOperator - 3), ',', false);
...
double FindOperator(char sExpression[], int iSymbolPosition, char cLastSymbol, bool bFindFirst)
{
	...
	if (bFindFirst)
	{
		...
		if (sExpression[iSymbolPosition - 1] == ')')
		{
			...
			for (int iCounter = iSymbolPosition - 2; iCounter >= 0; iCounter--)
			{
		...
		else
		{
			...
			for (int iCounter = iSymbolPosition - 1; iCounter >= 0; iCounter--)
			{
	...
	else
	{
		...
		if (sExpression[iSymbolPosition + 1] == '(')
		{
			...
			for (int iCounter = iSymbolPosition + 2; sExpression[iCounter] != '\0'; iCounter++)
			{
				if (sExpression[iCounter] == ')') break;
			}
		...
		else
		{
			...
			for (int iCounter = iSymbolPosition + 1; sExpression[iCounter] != '\0'; iCounter++)
			{
				if (IsNumber(sExpression, iCounter, true, true) == true) continue;
				else if (sExpression[iCounter] == 'e' || sExpression[iCounter - 1] == 'e') continue;
...
void ReplaceString(char sExpression[], char sNewString[], int iStart, int iEnd)
{
	...
	for (int iContatore2 = 0; sNewString[iContatore2] != '\0'; iContatore2++)
	{
...
void SortParenthesis(char sExpression[])
{
  ...
  for (int i = 0; (char)sExpression[i] != '\0'; i++)
  {
